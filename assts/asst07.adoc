...
return address of a function to jump to the `endGame` function.  In this question, we 
will analyze the binary executable using GDB to extract the secret codes explicitly. 

* link:https://diveintosystems.org/book/C7-x86_64/_attachments/secretx86-64.tar.gz[Download the binary]
* link:https://diveintosystems.org/book/C3-C_debug/gdb_assembly.html#_debugging_assembly_code[Debugging assembly code]
...
Modify the above contents to execute the instruction `push %rbp`. What two steps does `push %rbp` do?
